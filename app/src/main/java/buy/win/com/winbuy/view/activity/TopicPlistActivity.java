...
public class TopicPlistActivity extends Activity implements View.OnClickListener {
    ...
    private void initView() {
        mToolbar = (Toolbar) findViewById(R.id.tv_topiclist_title);
    ...
    private void openIntent() {
        ...
        if (topicBean != null) {
            mToolbar.setTitle(topicBean.name);

            String id = String.valueOf(topicBean.id);
            mPlistActivityPresenter.loadTopicPlistData(id, "saleDown");
            return;
        } else {
            String intientValue = intent.getStringExtra("intentKey");
            if ("newproductValue".equals(intientValue)) {
                mPlistActivityPresenter.loadNewProductData("saleDown");
                mToolbar.setTitle("新品上架");
            }
            if ("hotproductValue".equals(intientValue)) {
                mPlistActivityPresenter.loadHotProductData("saleDown");
                mToolbar.setTitle("热门单品");
            }
        }
    }
